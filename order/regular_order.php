<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $pricing = [
            ...
            'Ironing' => 4000,
            'Bed Sheet' => 7000,  // Changed from 'Bed sheet' to match JS
            'Bed Cover' => [      // Changed from 'Bed Cove' to match JS
                'S' => 20000,
                'M' => 25000,
                'XL' => 30000
            ],
            ...
            for ($i = 0; $i < count($order_types); $i++) {
                ...
                if (isset($items[$i]) && is_array($items[$i])) {
                    ...
                    for ($j = 0; $j < count($items[$i]); $j++) {
                        ...
                        // Handle single-value and array-based pricing
                        if (isset($pricing[$order_type])) {
                            if (is_array($pricing[$order_type])) {
                                $price_per_item = isset($pricing[$order_type][$item]) ? $pricing[$order_type][$item] : 0;
                            } else {
                                $price_per_item = $pricing[$order_type]; // Use the single value directly
                            }
                        } else {
                            $price_per_item = 0; // Default to 0 if pricing is not defined
                        }
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO order (customer_name, order_details, total_amount) VALUES (?, ?, ?)");
            $stmt->bind_param("ssd", $customer_name, $order_details_json, $total_price);
        } catch (Exception $e) {
            // If you don't have total_amount column
            $stmt = $conn->prepare("INSERT INTO order (customer_name, order_details) VALUES (?, ?)");
